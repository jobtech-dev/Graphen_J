...
import it.jobtech.graphenj.core.strategy.PeopleEtlStrategy
...
class JtEtlJobTest extends AnyFunSuite with Matchers with BeforeAndAfterEach with BeforeAndAfterAll {
  ...
  test("process all without an existing bookmark table") {
    ...
    // job running
    new JtEtlJob(jtConfiguration, new PeopleEtlStrategy, None).run()
  ...
  test("process without a source table") {
    ...
    // job running
    val res  = Try(new JtEtlJob(jtConfiguration, new PeopleEtlStrategy, None).run())
  ...
  test("process with an error during the writing") {
    ...
    // job running
    val res  = Try(new JtEtlJob(jtWrongConfiguration, new PeopleEtlStrategy, None).run())
    res.isFailure shouldBe true
    val left = res.toEither.left.get
